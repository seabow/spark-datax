...
import org.apache.spark.sql.catalyst.expressions.{BinaryExpression, ExpectsInputTypes, Expression, SpecificInternalRow}
...
private[sql] case class EvolutionProtobufDataToCatalyst(
                                                         left: Expression,
                                                         right: Expression,
                                                         messageName: String,
                                                         schema:DataType,
  ...
  @transient private lazy val messageDescriptorMap =
    mutable.Map.empty[String,Descriptor]

  @transient private lazy val fieldsNumbersMap =
    mutable.Map.empty[String,Set[Int]]

  @transient private lazy val deserializerMap =
    mutable.Map.empty[String,ProtobufDeserializer]

  private def loadNewProtoVersion(protoDescFile:String):Unit={
    if(!messageDescriptorMap.contains(protoDescFile)){
      val binaryFileDescriptorSet:Option[Array[Byte]]=Some(ProtobufUtils.readDescriptorFileContent(protoDescFile))
      val messageDescriptor=ProtobufUtils.buildDescriptor( messageName,binaryFileDescriptorSet)
      messageDescriptorMap.put(protoDescFile,
        messageDescriptor)
      fieldsNumbersMap.put(protoDescFile, messageDescriptor.getFields.asScala.map(f => f.getNumber).toSet)
     ...
     val newDeserializer= new ProtobufDeserializer(
      ...
      deserializerMap.put(protoDescFile,newDeserializer)
    }
  }
  ...
  override def nullSafeEval(input1: Any,input2:Any): Any = {
    ...
    try {
      if(!messageDescriptorMap.contains(protoDescFile)){
        loadNewProtoVersion(protoDescFile)
      }
      result = UncheckedDynamicMessage.parseFrom(messageDescriptorMap(protoDescFile), binary)
      ...
      result.getUnknownFields.asMap().keySet().asScala.find(fieldsNumbersMap(protoDescFile).contains(_)) match {
        case Some(number) =>
          ...
          throw QueryCompilationErrors.protobufFieldTypeMismatchError(
            messageDescriptorMap(protoDescFile).getFields.get(number).toString)
        case None =>
      }

      val deserialized = deserializerMap(protoDescFile).deserialize(result)
      assert(
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    nullSafeCodeGen(
      ...
      (eval1,eval2) => {
        ...
        s"""
        $dt $result = ($dt) $expr.nullSafeEval($eval1,$eval2);
        if ($result == null) {
          ${ev.isNull} = true;
        } else {
          ${ev.value} = $result;
  ...
  override protected def withNewChildrenInternal(newLeftChild: Expression,newRightChild: Expression): EvolutionProtobufDataToCatalyst =
    copy(left = newLeftChild, right = newRightChild)
