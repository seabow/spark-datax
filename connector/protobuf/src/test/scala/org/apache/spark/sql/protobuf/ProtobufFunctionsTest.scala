...
class ProtobufFunctionsTest extends QueryTest with SharedSparkSession with ProtobufTestBase
  ...
  private def from_protobuf_wrapper(
    ...
    options: Map[String, String] = Map("castEnumAsInt"->"false")): Column = {
    descBytesOpt match {
    ...
    checkWithFileAndClassName("SimpleMessageRepeated") {
      case (name, descFilePathOpt) =>
        List(
          ...
          Map("castEnumAsInt" -> "false"),
          Map("castEnumAsInt" -> "true")).foreach(opts => {
  ...
  test("test explicitly set zero values - proto3") {
    ...
    // With the emitDefaultValues flag set, we'll fill in
    ...
    checkWithFileAndClassName("Proto3AllTypes") { case (name, descFilePathOpt) =>
      ...
      checkAnswer(
        explicitZero.select(from_protobuf_wrapper(
          ...
          name,
          descFilePathOpt,
          Map("emitDefaultValues" -> "true","castEnumAsInt"->"false")).as("proto")),
        expectedWithFlag)
    }
  }
  ...
  test("test unset fields - proto2 types") {
    ...
    // emitDefaultValues will not materialize values for fields with presence
  ...
  test("test enum deserialization") {
    ...
    checkWithFileAndClassName("SimpleMessageEnum") { case (name, descFilePathOpt) =>
      List(Map.empty[String, String], Map("castEnumAsInt"->"true")).foreach(opts => {
        checkAnswer(
          message.select(
      ...
      checkAnswer(
        message.select(from_protobuf_wrapper(
          ...
          name,
          descFilePathOpt,
          Map("castEnumAsInt" -> "false")).as("proto")),
        expectedWithOption)
    }
  }
